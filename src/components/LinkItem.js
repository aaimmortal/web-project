...
class LinkItem extends React.Component {
    ...
    render() {
        return (
            <NavLink onClick={() => this.props.goto('GOTO', this.state.menuItem.path)}
                     key={this.state.menuItem.id}
                     to={this.state.menuItem.path == null ? "#" : this.state.menuItem.path}
                ...
                <div className={this.state.current === this.state.menuItem.path ? styles.activeItem : ''}>
                    <div className={"d-flex justify-content-between align-items-center"}>
                        <div className={"d-flex align-items-end"}>
                            ...
                            <div className={styles.icon_text}>{this.state.menuItem.name}</div>
                        </div>
                    ...
                    <div className={`${styles.sub} ${this.state.menuItem.open && styles.activeSub}`}>
                        {
                            this.state.menuItem.sub.map(subMenu => (
                                <LinkItem menuItem={subMenu} goto={this.props.goto} menuItems={this.props.menuItems}
                                          current={this.props.current}/>
                            ))
                        }
                    </div>
